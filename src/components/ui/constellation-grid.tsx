...
export default function ConstellationGrid() {
    ...
    const isDarkMode = true; // Forced dark mode for Marketix theme
    ...
    useEffect(() => {
        ...
        const handleResize = () => {
            ...
            // Get parent dimensions instead of window
        ...
        const render = (now: number) => {
            ...
            for (let i = 0; i < nodes.length; i++) {
                ...
                // We removed the physical attraction to keep the grid neutral and stable.
                // It only reacts visually now (opacity, size).

                // Slow organic drift around base position (Lissajous curve)
                const driftX = n.baseX + Math.cos(n.pulse * 0.2) * 25;
                const driftY = n.baseY + Math.sin(n.pulse * 0.3) * 25;

                // Calculate restoring force back to drifting anchor point
                const homeDx = driftX - n.x;
                const homeDy = driftY - n.y;
            ...
            for (let i = 0; i < nodes.length; i++) {
                ...
                // Removed noisy radar rings and hex labels to keep the UI clean and professional (Ponytail style).
            }
    ...
    return (
        <div className="absolute inset-0 z-0 overflow-hidden select-none bg-slate-950 dark:bg-slate-950 light:bg-slate-50">
